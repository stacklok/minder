...
SELECT s.id::uuid AS evaluation_id,
 ...
 WHERE (sqlc.narg(next)::timestamp without time zone IS NULL OR sqlc.narg(next) > s.most_recent_evaluation)
   ...
   AND (sqlc.slice(ruleTypes)::text[] IS NULL OR rt.name = ANY(sqlc.slice(ruleTypes)::text[]))
   ...
   AND (sqlc.slice(notRuleTypes)::text[] IS NULL OR rt.name != ANY(sqlc.slice(notRuleTypes)::text[]))
